package com.hyperion.methodmonitor.plugin.asm;
...
import com.hyperion.methodmonitor.plugin.CompressUtil;
import com.hyperion.methodmonitor.plugin.IMonitorProcessor;
            ...
            if (input.getJarInputs() != null) {
                for (JarInput jarInput : input.getJarInputs()) {
                    ...
                    try {
                        ...
                        // 先解压
                        CompressUtil.deCompress(srcPath, deCompressPath);
                        // 处理class文件
                        hook(deCompressPath, classLoader);
                        // 再压缩到目标文件
                        CompressUtil.compress(deCompressPath, destFile.getAbsolutePath());
                    } catch (Throwable e) {
                        e.printStackTrace();
                        // 出现异常，直接将原目录拷贝过去
                        try {
